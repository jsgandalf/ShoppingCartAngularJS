...
// shopping cart
//
function shoppingCart(cartName, returnUrl) {
...
shoppingCart.prototype.getTaxes = function (sku) {
    var total = 0;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (sku == null || item.sku == sku) {
            total += this.toNumber(item.quantity * item.price * .047);
        }
    }
    return total;
}
shoppingCart.prototype.getTotalPriceWithTaxes = function (sku) {
    var total = 0;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (sku == null || item.sku == sku) {
    ...
    return total+(total * .047);
}
shoppingCart.prototype.getTotalPriceWithTaxesWithShipping = function (sku) {
    var total = 0;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (sku == null || item.sku == sku) {
            total += this.toNumber((item.quantity * item.price) + (item.quantity * item.price * .047));
        }
    }
    return total +13;
}
...
shoppingCart.prototype.checkoutPayPal = function (parms, clearCart) {
    ...
    for (var i = 0; i < this.items.length; i++) {
		...
		data["no_shopping_" + ctr] = 2;
		data["return"] = this.returnUrl;
    }
		var taxes = 0;
		for (var i = 0; i < this.items.length; i++) {
			var item = this.items[i];
			taxes += this.toNumber(item.quantity * item.price);
		}
    	taxes =  (.047*taxes);
		ctr = i + 1;
        data["item_number_" + ctr] = "United States Taxes";
        data["item_name_" + ctr] = "Taxes";
        data["amount_" + ctr] = taxes;
		data["no_shopping_" + ctr] = 0;
		data["return"] = this.returnUrl;
		
		ctr = i + 2;
		data["item_number_" + ctr] = "Shipping";
        data["item_name_" + ctr] = "Shipping via Flat Rate";
        data["amount_" + ctr] = 13;
		data["no_shopping_" + ctr] = 0;
		data["return"] = this.returnUrl;
